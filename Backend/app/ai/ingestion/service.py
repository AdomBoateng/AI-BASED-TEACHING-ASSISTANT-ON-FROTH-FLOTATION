...
def _resolve_default_mode(default_mode: str | None, doc_type: str) -> str:
    mode_map = {"knowledge": "learn", "assessment": "review", "practice": "practice"}
...
def _resolve_doc_type(doc_type: str | None) -> str:
    ...
    synonyms = {"learn": "knowledge", "review": "assessment"}
...
async def ingest_document(
    file, doc_type, default_mode, difficulty, version, user_id, background_tasks
):
...
async def process_document_background(
    ...
    try:
        ...
        pinecone_ids = await asyncio.to_thread(
            upsert_vectors, embeddings, chunks, namespace=f"froth-{default_mode}"
        )
