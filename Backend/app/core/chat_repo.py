...
def get_session_prefers_video(session_id: str) -> bool:
    supabase = get_supabase()
    res = supabase.table("sessions").select("prefers_video").eq("id", session_id).single().execute()
    return bool(res.data.get("prefers_video")) if res.data else False

def response_format_for_mode(session_id: str, mode: str) -> str:
    """Enforce: review is text-only; learn/practice follow sessions.prefers_video."""
