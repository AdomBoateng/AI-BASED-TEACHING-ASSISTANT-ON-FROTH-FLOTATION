...
import { Menu, Moon, Sun, Video, FileText, Loader2, BookOpen, FlaskConical, ClipboardCheck } from 'lucide-react';
...
const MODE_TABS: { mode: TutorMode; label: string; icon: React.ElementType; color: string; activeColor: string }[] = [
  { mode: 'learn', label: 'Learn', icon: BookOpen, color: 'text-muted-foreground', activeColor: 'text-blue-400' },
  { mode: 'practice', label: 'Practice', icon: FlaskConical, color: 'text-muted-foreground', activeColor: 'text-emerald-400' },
  { mode: 'review', label: 'Review', icon: ClipboardCheck, color: 'text-muted-foreground', activeColor: 'text-amber-400' },
];
...
export function Header() {
  ...
  const handleModeClick = async (mode: TutorMode) => {
    ...
    if (mode === 'learn') {
      // Switch back to learn immediately (no modal needed)
      await switchMode('learn');
    } else {
      // Practice or Review need the config modal
      setModeSelectorTarget(mode as 'practice' | 'review');
    }
  };
  ...
  const handleSetVideoMode = async (videoMode: boolean) => {
    ...
    if (isReviewMode) return; // enforced by backend anyway
